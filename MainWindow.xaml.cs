...
namespace LegendBar
{
    public sealed partial class MainWindow : Window
    {
        ...
        private AddModePopup? _addModePopup;
        private ViewModesPopup? _viewModesPopup;
        ...
        private void SetupWindow()
        {
            ...
            _modeService = new ModeService(DispatcherQueue);
            ClockWidgetContainer.ModeService = _modeService;
            _modeService.ModeActivated += mode =>
                DispatcherQueue.TryEnqueue(() =>
                    ClockWidgetContainer.ActivateMode(mode));
            _modeService.ModeDeactivated += () =>
                DispatcherQueue.TryEnqueue(() =>
                    ClockWidgetContainer.DeactivateMode());

            ClockWidgetContainer.AddModeRequested += OpenAddMode;
            ClockWidgetContainer.ViewModesRequested += OpenViewModes;
        ...
        {
            if (_addModePopup != null) return;
            if (!_isPinned) _autoHide?.ForceShow();
            _addModePopup = new AddModePopup(_modeService!);
            _addModePopup.Closed += (s, e) =>
            {
                _addModePopup = null;
                _autoHide?.SetExternalWindowOpen(false);
            };
            _autoHide?.SetExternalWindowOpen(true);
            _addModePopup.Activate();
        }

        private void OpenViewModes()
        {
            if (_viewModesPopup != null) return;
            if (!_isPinned) _autoHide?.ForceShow();
            _viewModesPopup = new ViewModesPopup(_modeService!);
            _viewModesPopup.Closed += (s, e) =>
            {
                _viewModesPopup = null;
                _autoHide?.SetExternalWindowOpen(false);
            };
            _viewModesPopup.ModeActivated += mode =>
                _modeService?.ManualActivate(mode);
            _autoHide?.SetExternalWindowOpen(true);
            _viewModesPopup.Activate();
        }
