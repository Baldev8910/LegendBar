...
namespace LegendBar
{
    public sealed partial class SettingsPopup : Window
    {
        ...
        private void SetupWindow()
        {
            ...
            int popupX = (primary?.LogicalBounds.Right ?? 1920) - 340;
            int popupY = SettingsService.Current.BarHeight + 8;
            _appWindow.MoveAndResize(new RectInt32(popupX, popupY, 320, 680));
        }
        ...
        private void Reset_Click(object sender, RoutedEventArgs _e)
        {
            ...
            SettingsService.Current.MaterialType = defaults.MaterialType;
            SettingsService.Current.TintColor = defaults.TintColor;
            SettingsService.Current.ShowMediaWidget = defaults.ShowMediaWidget;
            SettingsService.Current.ShowClock = defaults.ShowClock;
            SettingsService.Current.ShowDate = defaults.ShowDate;
        ...
        private void TempUnit_Changed(object sender, RoutedEventArgs _e)
        {
            if (_loading) return;
        ...
        private void Startup_Toggled(object sender, RoutedEventArgs _e)
        {
            if (_loading) return;
        ...
        private void MediaToggle_Toggled(object _sender, RoutedEventArgs _e)
        {
            if (_loading) return;
        ...
        private void ClockToggle_Toggled(object sender, RoutedEventArgs _e)
        {
            if (_loading) return;
        ...
        private void DateToggle_Toggled(object sender, RoutedEventArgs _e)
        {
            if (_loading) return;
